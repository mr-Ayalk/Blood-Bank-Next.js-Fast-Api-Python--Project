...
export default function AdminDonors() {
  ...
  // frontend/app/dashboard/admin/donors/page.tsx
  useEffect(() => {
    apiRequest("/admin/donors/requests").then(setDonors);
  }, []);

  // In your return, display the specific donation details
  {
    donors.map((d) => (
      <div key={d.DonationRequest.id}>
        {d.UserProfile.full_name} wants to donate{" "}
        {d.DonationRequest.blood_group}
      </div>
    ));
  }
    ...
    <div className="p-8 ml-64 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="relative">
          ...
          />
          <input
            className="pl-10 pr-4 py-2 border rounded-xl outline-none focus:ring-2 focus:ring-red-500"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {donors.map((donor) => (
          ...
          >
            ...
            <button className="text-xs font-semibold bg-gray-100 px-3 py-1 rounded-lg hover:bg-gray-200">
              View History
            </button>
          </div>
        ))}
